...
#+CAPTION: will be deployed the requested OpenStack stable/rocky
#+CAPTION: services.
...
** Update with a new version of EnOS                               :noexport:
To update this when a new version of EnOS appears change in the
following the values of ~new-os-version~ and ~new-enos-install~ with
the value of the next release. Then executes the code with ~C-c C-c~.
