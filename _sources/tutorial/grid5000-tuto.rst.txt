...
- An SSH client (*e.g.*, OpenSSH on Linux/Mac, Putty on Windows)
  ...
  - Follow the `G5K’s recommendations <https://www.grid5000.fr/w/SSH#Using_SSH_ProxyCommand_feature_to_ease_the_access_to_hosts_inside_Grid.275000>`_ and edit your ``~/.ssh/config``
...
Here, we get a new machine in interactive mode (*i.e.*, ``-I``) for the
...
- ``resources`` section (l. 13): Defines the number and role of
  machines to deploy on the testbed (*e.g.*, book 3 nodes on ``paravance``
...
.. note::
    ...
    Refer to the `Grid’5000 provider documentation <https://beyondtheclouds.github.io/enos/provider/grid5000.html>`_ for more details.
...
browser (from your laptop), you can either use the `Grid’5000 VPN <https://www.grid5000.fr/w/VPN>`_, or create a SSH tunnel from your
...
.. note::
    ...
    This lab has been designed to **run on a cluster where nodes have two network interfaces**. **If you plan to run the lab on a cluster with a single network interface**, **please run the following script on the network node**. You can check how many network interfaces are associated to a
    cluster by consulting the `Hardware page for the site you’re using <https://www.grid5000.fr/w/Rennes:Hardware>`_. If your
...
.. [9] `https://www.grid5000.fr/w/Advanced_OAR#Changing_the_walltime_of_a_running_job_.28oarwalltime.29 <https://www.grid5000.fr/w/Advanced_OAR#Changing_the_walltime_of_a_running_job_.28oarwalltime.29>`_
