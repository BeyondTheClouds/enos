...
def check_glance(session, image_name):
    ...
    if image_name not in map(itemgetter('name'), name_ids):
        logging.error("[glance]: Image %s is missing in the available images" %  image_name)
        raise Exception("Image %s is missing in the available images" %  image_name)
    else:
...
def check_network(session, configure_network, network, subnet,  dns_nameservers=None, allocation_pool=None,
    ...
    if secgroup_name not in map(itemgetter('name'), secgroups):
        ...
        logging.info("[neutron]: %s security group has been created on OpenStack" % secgroup_name)
        # create the rules
        tcp = {
            'protocol': 'tcp',
            'direction': 'ingress',
        ...
        logging.info("[neutron]: %s security group rule (all tcp) has been created on OpenStack" % secgroup_name)
        nclient.create_security_group_rule({'security_group_rule': icmp})
        logging.info("[neutron]: %s security group rule (all icmp)\ has been created on OpenStack" % secgroup_name)
    else:
        logging.info("[neutron]: Reusing existing security-groups %s " % secgroup_name)
    ...
    if network_name not in map(itemgetter('name'), networks):
        ...
        network  = res['network']
        logging.info("[neutron]: %s network has been created on OpenStack" % network_name)
    else:
    ...
    if subnet_name not in map(itemgetter('name'), subnets) and configure_network:
        ...
        logging.debug(s)
        subnet = s['subnet']
        logging.info("[neutron]: %s subnet has been created on OpenStack" % subnet_name)
    else:
        subnet = [s for s in subnets if s['name'] == subnet_name][0]
        logging.info("[neutron]: Reusing existing %s subnet", subnet)
    ...
    if router_name not in map(itemgetter('name'), routers['routers']) and configure_network:
        ...
        logging.info("[neutron]  %s router has been created on Openstack" % router_name)
...
def check_servers(session, resources, extra_prefix="",
    ...
    """
    nclient = nova.Client(NOVA_VERSION, session=session)
    servers =  nclient.servers.list(search_opts={'name': '-'.join([PREFIX, extra_prefix])})
#    servers = filter(lambda s: is_in_current_deployment(s, extra_prefix=extra_prefix), servers)
    ...
    if len(servers) == wanted_number:
        ...
        logging.error("[nova]: Not enough servers to reuse : found %s / wanted %s" % (len(servers), wanted_number))
        raise Exception('Not enough servers to reuse')
    else:
        userdata = open(os.path.join(CURRENT_DIR, 'openstack.sh'), 'r')
        total = 0
...
def check_gateway(env, with_gateway, servers):
    ...
    if with_gateway:
        ...
        gateway_floating_ips = filter(lambda n: n['OS-EXT-IPS:type'] == 'floating', gateway.addresses[env['network']['name']])
        if len(gateway_floating_ips) == 0:
            floating_ip = nclient.floating_ips.create(env['ext_net']['name'])
            gateway.add_floating_ip(floating_ip)
            gateway = nclient.servers.get(gateway.id)
